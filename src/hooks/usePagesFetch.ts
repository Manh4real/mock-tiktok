...
const usePagesFetch = <T extends { id: number }>(
  ...
  const { reachEndFunc = () => {}, onSuccess } = options;
  ...
  const handleFetchNext = () => {
    ...
    fetchApi(page + 1)
      ...
      .catch(() => {
        ...
        dispatch(show({ message: options.errorMessage || "" }));
...
// ==================================================================================
export const usePagesFetch__videos = <T extends Video>(
  fetchApi: FetchApiFunc<T>,
  pauseCallCheck: boolean = false,
  options: Partial<{
    errorMessage: string;
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      const newData = data.data.map((video: Video) => {
        ...
        if (
          newVideo.allows instanceof String ||
          typeof newVideo.allows === "string"
        ) {
